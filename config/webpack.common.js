...
module.exports = {
  ...
  output: {
    ...
    publicPath: '/', // 实际中根据自己网站的url改写这里，主要为正确加载require.ensure的内容（for多入口网站）
  ...
  plugins: [
    ...
    //   context: ROOT_PATH,
    //   manifest: require('../lib_bundle/manifest.json')
    // }),
    // 提取公用库，提升缓存/编译效率
    ...
    //// 生成其它入口html文件
    //new HtmlWebpackPlugin({
    //  title: info.other.title,
    //  description: info.other.description,
    //  keywords: info.other.keywords,
    //  template: path.join(SRC_PATH, 'entries/app.html'),
    //  filename: 'other/index.html', // 生成的html文件
    //  chunks: ['other', 'lib'],
    //  favicon: path.join(SRC_PATH, 'assets/favicon.png')
    //}),
    // 抽取css文件
